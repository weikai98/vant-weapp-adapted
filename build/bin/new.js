...
const COMPONENTTYPE = ['basic', 'form', 'notice', 'data', 'other']
const componentType = COMPONENTTYPE.includes(process.argv[4])
  ? process.argv[4]
  : 'basic'
...
const examplePagePath = path.resolve(
  __dirname,
  `${example}/pages/${componentname}`
)
fs.mkdir(examplePagePath, (err) => {
  if (!err) {
    exampleFiles.forEach((file) => {
      fs.writeFileSync(
        path.resolve(examplePagePath, file.filename),
        file.content
      )
    })
  }
...
const examplaMenuItem = exampleMenuList.find(
  (menuItem) => menuItem.type === componentType
)
...
const exampleMenuListPath = path.resolve(
  __dirname,
  `${example}/common/menuList.js`
)
...
// 组件
vuepressConfig.themeConfig.sidebar[1].children.push(
